...
print(json.dumps(row))`
        ...
        // Heuristic fixer for tokens where 's' was stripped by upstream CSVs (e.g., "java cript" -> "javascript").
        function repairTokenS(x) {
          let t = String(x || '').trim();
          if (!t) return t;
          // Common repairs
          t = t.replace(/\bjava\s*cript\b/gi, 'javascript')
               .replace(/\bnode\.j\b/gi, 'node.js')
               .replace(/\bpower\s*hell\b/gi, 'powershell')
               .replace(/\bwordpre\b/gi, 'wordpress')
               .replace(/\bcyber\s*ecurity\b/gi, 'cyber security')
               .replace(/\binformation\s*ecurity\b/gi, 'information security')
               .replace(/\bde\s*ktop\b/gi, 'desktop')
               .replace(/\bmicro\s*oft\b/gi, 'microsoft')
               .replace(/\bpri?ng\b/gi, 'spring')
               .replace(/\bci\s*ty\b/gi, 'city')
               .replace(/\b\s*tate\b/gi, 'state');
        ...
        try {
          ...
          if (csvPath) {
            const peerRaw = pyJson('python3', ['-c',
...
print(json.dumps(terms))`
            ...
            const peer = peerRaw.map(repairTokenS).filter(Boolean);
...
`,
            ...
            const recRaw = pyJson('python3', ['-c', py], { encoding: 'utf-8' }) || [];
            const rec = recRaw.map(repairTokenS).filter(Boolean);
            const resumeText = (textRow && textRow.text ? String(textRow.text).toLowerCase() : '');
