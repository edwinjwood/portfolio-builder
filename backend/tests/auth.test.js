...
describe('Auth routes', () => {
  ...
  test('POST /api/login returns 401 with invalid creds', async () => {
    ...
    const res = await request(app).post('/api/login').send({ email: 'noone@example.com', password: 'x' });
  ...
  test('POST /api/login succeeds with valid creds', async () => {
    ...
    const res = await request(app).post('/api/login').send({ email: 'u@example.com', password: 'Password123' });
