...
export function AuthProvider({ children }) {
	...
	const signup = async ({ email, password, name, plan, username } ) => {
		...
		try {
			...
			const existsLocal = demoUsers.find((u) => u.email === email);
			if (exists || existsLocal) throw new Error('Email already registered');
...
async function validateToken() {
	...
	try {
		const apiUrl = import.meta.env.VITE_API_URL || '';
		// Backend mounts validate under /api/users/validate
		const res = await fetch(`${apiUrl}/api/users/validate`, {
			method: 'POST',
